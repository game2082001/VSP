...
```json
{
  ...
  "claudeArtifactDeveloper": {
    "required": false,
    "status": "NOT_REQUESTED|PENDING_DEFAULT_BRANCH|AVAILABLE|OPERATIONAL|FAILED",
    "workflowPath": "",
    "workflowRunId": "",
    "workflowRunAttempt": "",
    "taskId": "",
    ...
    "packageSha256": "",
    "changedFiles": [],
    "claudeExecutionInGitHubActions": false,
    "developerRepositoryWriteCredential": false,
    "repositoryWriteCredentialAvailableToClaude": false,
    "productOwnerManualRelay": false,
    "modelMediatedFileTransfer": false,
    "realClaudeDeveloperRunProven": false
  },
  "remainingKnownRisks": [],
  "scopeDrift": "NONE",
  "readyForMerge": false,
...
`claudeArtifactDeveloper` records the AI02 Claude developer artifact boundary introduced by VSP-AI02-001TI-B1. Before B1 merges, the only valid status for B1 evidence is `PENDING_DEFAULT_BRANCH`: the workflow is not yet available on default branch, Claude developer execution is not operational, and `realClaudeDeveloperRunProven` must remain `false`. A later Product Owner-authorized smoke may advance this to `OPERATIONAL` only after a real Claude GitHub Actions run produces an artifact with digest evidence and no repository write credential exposure.
