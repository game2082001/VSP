...
| User / Role / Permission | Core | Management Server | Authenticates, acts within granted role | Sole authority for identity/roles (Vision §21) | Future | No | N/A — Core, always present | **Implemented** (v1.0, Epic-018) — local username/password, fixed Admin/Operator roles only, single standalone-desktop process; no LDAP/OAuth/JWT/MFA/SSO, no User Management UI, no multi-user/remote sessions. Vision §21's full Management-Server-owned identity/session authority model remains Future, unaffected by this v1.0 scope | — |
...
| Backup / Restore | Core | Management Server | Triggers/monitors backup or restore | Backs up/restores platform state it owns (Vision §14) | No | No | N/A — Core, always present | **Implemented** (v1.0, Epic-017) — manual Backup/Restore of the single SQLite database only, single standalone-desktop process; no scheduled/cloud backup, no recording-file/settings-file backup, no encryption, no backup-history management. Vision §14's full Management-Server-owned platform-state backup/restore model remains Future, unaffected by this v1.0 scope | — |
