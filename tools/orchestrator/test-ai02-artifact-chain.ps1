...
try {
    ...
    $crossBaseA2Val1=New-Package (Join-Path $root "cross-base-a2-val1") A2 "sha256:a29ea66e53f3645ca38c0b2b6e2880cb472a3f37bc1fea15b01980bea2a2caa1" "VSP-AI02-001TI-A2-VAL1" @{} @() $a2ExecutionSha
    $a2Val1ResultPath=Join-Path $crossBaseA2Val1.Root "publication-package.result.json"
    Expect-Pass "approved A2 VAL1 replacement child identity is accepted narrowly" { Invoke-Chain (Merge-Arguments $a2CheckpointArgs @{ChildDescriptorPath=$crossBaseA2Val1.DescriptorPath;ChildPackageResultPath=$a2Val1ResultPath;CheckpointPath=(Join-Path $root "a2-val1-checkpoint.json")}) }
    ...
    $wrongReplacementTask=Copy-CanonicalJsonWithMutation $crossBaseA2Val1.DescriptorPath (Join-Path $root "wrong-replacement-task.json") { param($v) $v.childTaskId="VSP-AI02-001TI-A2-VAL2" }
    $wrongReplacementResult=Copy-CanonicalJsonWithMutation $a2Val1ResultPath (Join-Path $root "wrong-replacement-result.json") { param($v) $v.taskId="VSP-AI02-001TI-A2-VAL2" }
    Expect-Fail "unapproved A2 replacement identity remains rejected" { Invoke-Chain (Merge-Arguments $a2CheckpointArgs @{ChildDescriptorPath=$wrongReplacementTask;ChildPackageResultPath=$wrongReplacementResult;CheckpointPath=(Join-Path $root "wrong-replacement-task.json")}) }
