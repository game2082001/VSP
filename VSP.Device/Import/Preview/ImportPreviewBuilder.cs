...
public class ImportPreviewBuilder
{
    ...
    private static ImportPreviewRow BuildRow(ImportValidationResult result)
    {
        return new ImportPreviewRow
        {
            ...
            HttpPort = GetValue(result, "HTTP Port"),
            RtspPort = GetValue(result, "RTSP Port"),
            SdkPort = GetValue(result, "SDK Port"),
            Username = GetValue(result, "Username"),
            ConnectionType = GetValue(result, "Connection Type"),
            RtspUrl = GetValue(result, "RTSP URL"),
